...
| **Mean Average Precision (MAP)**    | Computes the average precision for each query and then averages these values over all queries.     | MAP = (1 ÷ ABS(Q) × SUM(Average Precision(q)) for q in Q                                                                                           |
...
| **Mean Reciprocal Rank (MRR)**      | Evaluates the rank position of the first relevant document.                                        | MRR = (1 ÷ |Q|) × SUM(1 ÷ rank_i) for i = 1 to |Q|                                                                                              |
...
| **Hit Rate (HR) or Recall at k**    | Measures the proportion of queries for which at least one relevant document is retrieved in the top k results. | HR@k = (Queries with at least one relevant document in top k) ÷ |Q|                                                                          |
| **Expected Reciprocal Rank (ERR)**  | Measures the probability that a user finds a relevant document at each position in the ranked list. | ERR = Σ(1 ÷ i) × Π(1 − r_j) × r_i for j = 1 to i−1<br>r_i = relevance probability of the document at position i.                            |
